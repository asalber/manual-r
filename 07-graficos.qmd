...
- `geom_histogram(stat_bin(bindwidth = anchura))`: Dibuja un histograma de la variable asociada al atributo `x` usando clases de amplitud indicada por `anchura`. Si no se indica el parámetro `bindwidth` se toman 30 clases por defecto.
